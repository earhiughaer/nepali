...
INPUT = Path("alphabet/Vokale.mp3")
OUT_DIR = Path("alphabet/vowels")
...
def main():
    duration = float(sys.argv[1]) if len(sys.argv) > 1 else 5.485688
    regions = [(i * duration / len(VOWELS), (i + 1) * duration / len(VOWELS)) for i in range(len(VOWELS))]
    ...
    manifest = []
    for idx, ((vowel, label), (start, end)) in enumerate(zip(VOWELS, regions), start=1):
